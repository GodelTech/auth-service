...
class TaskSetDeviceEndpoint(SequentialTaskSet):
    content_type = 'application/x-www-form-urlencoded'

    @task(5)
    def test_successful_post_device_authorize(self) -> None:
        params = {
            "client_id": "test_client",
            "scope": "scope"
        }
        self.client.request("POST", "/device/",
                            data=params,
                            headers={'Content-Type': self.content_type},
                            name="/device")

    # def on_stop(self, connection=connection):
    ...
    @task
    async def test_unsuccessful_post_device_authorize(self) -> None:
        params = {
    ...
    @task
    async def test_get_device_cancel(self) -> None:
        self.client.request("GET", "/device/auth/cancel",
                            name="/device/auth/cancel")
