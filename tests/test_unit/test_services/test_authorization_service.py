...
class TestAuthorisationService:
    ...
    async def test_get_redirect_url(self, authorisation_service, authorization_request_model):
    ...
    async def test_get_redirect_url_wrong_client(self, authorisation_service, authorization_request_model):
        service = authorisation_service
        service.request_model = authorization_request_model
    ...
    async def test_get_redirect_url_wrong_scope_key(self, authorisation_service, authorization_request_model):
        service = authorisation_service
        service.request_model = authorization_request_model
    ...
    async def test_get_redirect_url_wrong_scope_index(self, authorisation_service, authorization_request_model):
        service = authorisation_service
        service.request_model = authorization_request_model
    ...
    async def test_get_redirect_url_wrong_scope_password(self, authorisation_service, authorization_request_model):
        service = authorisation_service
        service.request_model = authorization_request_model
    ...
    async def test_get_redirect_url_wrong_scope_user_name(self, authorisation_service, authorization_request_model):
        service = authorisation_service
        service.request_model = authorization_request_model
        scope = 'gcp-api%20IdentityServerApi&grant_type=' \
                'password&client_id=test_client&client_secret=' \
    ...
    async def test_validate_client(self, authorisation_service):
        await authorisation_service.client_repo.session.execute(insert(Client).values(**DEFAULT_CLIENT))
        client = await authorisation_service._validate_client(client_id='default_test_client')
        assert client is True

    async def test_validate_client_error(self, authorisation_service):
        with pytest.raises(ClientNotFoundError):
            await authorisation_service._validate_client(client_id='test_client_not_exist')
    ...
    async def test_update_redirect_url_with_state(self, authorisation_service, authorization_request_model):
        authorisation_service.request_model = authorization_request_model
        expected_url = 'https://www.google.com/?code=secret&state=state'
        redirect_url = await authorisation_service._update_redirect_url_with_params('secret')

        assert redirect_url == expected_url

    async def test_update_redirect_url_without_state(self, authorisation_service, authorization_request_model):
        authorisation_service.request_model = authorization_request_model
