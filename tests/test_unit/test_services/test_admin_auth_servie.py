...
class TestAdminAuthService:
    ...
    async def test_authorize(self, admin_auth_service: AdminAuthService) -> None:
        service = admin_auth_service
        credentials = AdminCredentialsDTO(username="TestClient", password="test_password")
        result = await service.authorize(credentials=credentials)
        assert result
        
        credentials = AdminCredentialsDTO(username="Vi", password="test_password")
        with pytest.raises(UserNotFoundError):
            await service.authorize(credentials=credentials)
        
        credentials = AdminCredentialsDTO(username="TestClient", password="samurai")
        with pytest.raises(WrongPasswordError):
            await service.authorize(credentials=credentials)

    async def test_authenticate(self, admin_auth_service: AdminAuthService) -> None:
        service = admin_auth_service
        payload={"aud": ["admin",], "exp":int(time())+1000}
        token = await JWTService().encode_jwt(payload)
        result = await service.authenticate(token=token)
        assert result

        payload={"aud": ["not_admin",], "exp":int(time())+1000}
        token = await JWTService().encode_jwt(payload)
        result = await service.authenticate(token=token)
        assert result is False

        payload={"aud": ["admin",], "exp":0}
        token = await JWTService().encode_jwt(payload)
        result = await service.authenticate(token=token)
        assert result is False
    
    async def test_authorize_authenticate(self, admin_auth_service: AdminAuthService) -> None:
        service = admin_auth_service
        credentials = AdminCredentialsDTO(username="TestClient", password="test_password")
        result = await service.authorize(credentials=credentials)
