...
class TestAdminRoleEndpoint:
    ...
    async def setup(self, engine, user_id:int = 1000): 
    ...
    async def test_get_role(self, engine, client:AsyncClient):
        ...
        params ={"role_id":(await self.role_repo.get_role_by_name("Standuser")).id}

        response = await client.request("GET", "/administration/role/get_role", headers=headers, params=params)
    ...
    async def test_get_all_roles(self, engine, client:AsyncClient):
        ...
        response = await client.request("GET", "/administration/role/get_all_roles", headers=headers)
        assert response.status_code == status.HTTP_200_OK
        response_content = json.loads(response.content.decode('utf-8'))
        logger.info(response_content)
    ...
    async def test_get_roles(self, engine, client:AsyncClient):
        ...
        params = {"role_ids":f'{(await self.role_repo.get_role_by_name(name = "Standuser")).id},{(await self.role_repo.get_role_by_name(name = "Vampire")).id}'}

        response = await client.request("GET", "/administration/role/get_roles", headers=headers, params=params)
        assert response.status_code == status.HTTP_200_OK
        response_content = json.loads(response.content.decode('utf-8'))
        logger.info(response_content)
    ...
    async def test_delete_role(self, engine, client:AsyncClient):
        ...
        params = {"role_id":(await self.role_repo.get_role_by_name(name = "Standuser")).id}

        response = await client.request("DELETE", "/administration/role/delete_role", headers=headers, data=params)
        assert response.status_code == status.HTTP_200_OK
        headers = {
            "access-token" : self.access_token
            }
    ...
    async def test_create_update_role(self, engine, client:AsyncClient):
        await self.setup(engine,)
        try:
            await self.role_repo.delete((await self.role_repo.get_role_by_name("Pillar Man")).id)
        except:
            pass
        try:
            await self.role_repo.delete((await self.role_repo.get_role_by_name("Ultimate Life Form")).id)
        except:
            pass
        ...
        response = await client.request("POST", "/administration/role/new_role", headers=headers, data=params)
        assert response.status_code == status.HTTP_200_OK
        ...
        response = await client.request("PUT", "/administration/role/update_role", headers=headers, data=params)
        assert response.status_code == status.HTTP_200_OK
