...
class TestThirdPartyGitLabFlow:
    async def test_successful_gitlab_code_flow(
        ...
        # Stage 2: Third party Gitlab provider endpoint request to collect username and access_token
        ...
        patch_start = "src.business_logic.third_party_auth.service_impls.gitlab.GitlabAuthService"
        mocker.patch(f"{patch_start}._get_access_token", replace_post)
        mocker.patch(f"{patch_start}._get_username", replace_get)
        gitlab_params = {"code": "test_code", "state": STUB_STATE}
        gitlab_response = await client.request(
            "GET", "/authorize/oidc/gitlab", params=gitlab_params
        )
        assert gitlab_response.status_code == status.HTTP_302_FOUND

        # Stage 3: Token endpoint changes secrete code in Persistent Grant table to token
        secret_code = (
            gitlab_response.headers["location"].split("=")[1].split("&")[0]
        )
        token_params = {
            "client_id": "spider_man",
