...
class TestAuthoriseEndpoint:
    ...
    async def test_unsuccessful_authorize_request_not_full_data(self, client: AsyncClient):
        params = {
            'response_type': 'code',
            'scope': scope,
            'redirect_uri': 'https://www.google.com/',
    ...
    async def test_unsuccessful_authorize_request_wrong_client(self, client: AsyncClient):
        params = {
    ...
    async def test_unsuccessful_authorize_request_bad_password(self, client: AsyncClient):
    ...
    async def test_unsuccessful_authorize_request_bad_user_name(self, client: AsyncClient):
        local_scope = 'gcp-api%20IdentityServerApi&grant_type=' \
                'password&client_id=test_client&client_secret=' \
                '65015c5e-c865-d3d4-3ba1-3abcb4e65500&password=' \
    ...
    async def test_unsuccessful_authorize_request_wrong_scope_index_error(self, client: AsyncClient):
        local_scope = 'gcp-api%20IdentityServerApi&grant_type'
        params = {
            'client_id': 'test_client',
            'response_type': 'code',
    ...
    async def test_unsuccessful_authorize_request_wrong_scope_key_error(self, client: AsyncClient):
        ...
        expected_content = '{"message":"The scope is missing a password, or a username"}'
        response = await client.request('GET', '/authorize/', params=params)
