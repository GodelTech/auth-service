...
class TestTokenEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_code_authorization(self, client: AsyncClient, connection):
        ...
        assert await self.persistent_grant_repo.exists(grant_type='refresh_token', data=pytest.refresh_token) == True


    @pytest.mark.asyncio
    async def test_refresh_token_authorization(self, client: AsyncClient, connection):
        '''
        ...
        assert await persistent_grant_repo.exists(grant_type='refresh_token', data=data) == True
        assert response.status_code == status.HTTP_200_OK


    @pytest.mark.asyncio
    async def test_wrong_client_id(self, client: AsyncClient, connection):
        ...
        response = await client.request('POST', '/token/', data=wrong_params, headers={'Content-Type': self.content_type})
