...
from src.data_access.postgresql import Client
...
class TokenService:
    ...
    async def get_tokens(self) -> Optional[Dict[str, Any]]:
        if self.request_model is not Ellipsis:
                ...
                if (
                    ...
                    if not await self.persistent_grant_repo.exists(
                        ...
                        if await self.device_repo.validate_device_code(
                            ...
                            expire_in = (
                                await self.device_repo.get_expiration_time(
                                    device_code=self.request_model.device_code
                                )
                            )
    ...
    async def get_client_credentials(self) -> Optional[Dict[str, Any]]:
        expiration_time = 600
        client_from_db = ...
        if self.request_model is not Ellipsis:
            try:
