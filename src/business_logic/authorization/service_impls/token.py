...
class TokenAuthService(UpdateRedirectUrlMixin):
    """
    Service for handling access token generation and redirect URL construction in an authentication system.
    ...
    def __init__(
        ...
        """
        Initialize the TokenAuthService.
    ...
    async def _validate_request_data(self, request_data: AuthRequestModel):
        """
        Validate the request data for access token generation.
    ...
    async def _get_access_token(
        ...
        """
        Generate an access token based on the provided request data, user ID, and Unix time.
    ...
    async def get_redirect_url(self, request_data: AuthRequestModel) -> str:
        """
        Get the redirect URL with the access token.

        Args:
            request_data: An instance of AuthRequestModel containing the request data.

        Returns:
            The redirect URL containing the access token.
