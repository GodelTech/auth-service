...
def provide_jwt_manager(sync_session: Session) -> JWTManagerProtocol:
    rsa_keys = provide_rsa_keys(sync_session=sync_session)
    return JWTManager(keys=rsa_keys)

# def provide_jwt_manager(session: Session) -> JWTManagerProtocol:
#     keys = provide_rsa_keys(session=session)
